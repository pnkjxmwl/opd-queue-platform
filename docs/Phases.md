...
| ✅ P9-WEB-02 | Staff pace panel on the queue board | WEB | 2 | — | **done, and not what this row originally said.** Admin reports are deferred; what shipped is the board finally SHOWING what the engine already knew - pace, what it is based on, handover time and whether today is running behind. `GET /sessions/:id/eta` had served all of it since Phase 7 with no screen asking. Found by auditing whether the ETA work connected, not by a failing test |
...
| ◐ P10-MOB-01 | Mobile → EAS build → TestFlight + Play internal | MOB | 2 | 02 | **Android done 2026-09-08**, stores deliberately not. A `preview` APK built on EAS (commit `59849f9`) and was installed and run on a real phone, pointed at the Render API. **TestFlight and Play internal are NOT done** - both need paid developer accounts the user has chosen not to buy yet, so this is a `◐` and must not be rounded up |
