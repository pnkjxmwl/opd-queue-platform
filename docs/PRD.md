...
5. **Unified queue** per session; each entry is typed (ONLINE / WALK_IN / FOLLOW_UP / PRIORITY / EMERGENCY); the Queue Policy decides ordering.
...
8. **No-show** (checked-in then absent): grace period → recall → SKIPPED → move to end (all configurable).
9. **Reservations that never arrive** are never called; at session end they become NO_SHOW, refund per policy.
10. **OPD session state** (SCHEDULED → OPEN_FOR_REGISTRATION → ACTIVE → COMPLETED; +CANCELLED/ENDED_EARLY) is **independent of doctor presence** (NOT_PRESENT → PRESENT → ON_BREAK → LEFT).
11. **Doctor late** → session/queue unaffected, ETA recalculates. **Doctor leaves early** → session ends, remaining patients rescheduled per policy. **Doctor substitution** → preserve original doctor + actual provider; notify patients.
