...
10. **OPD session state** (SCHEDULED → OPEN_FOR_REGISTRATION → ACTIVE → COMPLETED; +CANCELLED/ENDED_EARLY) is **independent of doctor presence** (NOT_PRESENT → PRESENT → ON_BREAK → LEFT). Two consequences settled in Phase 4: there is no separate "start session" command — the first `call-next` moves a session to ACTIVE — and presence is recorded, never validated, with one exception: `call-next` is refused while presence is LEFT, because §8.11 says a departed doctor ends the session and calling more patients into an empty room is a forgotten step rather than a workflow.
